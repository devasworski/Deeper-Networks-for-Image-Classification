...
\end{figure}
In Figure \ref{fig:x mode size} it can be observed, that the model size is in between GoogLeNet and VGG-16, yet the best result can be expected according to this comparision.
ResNet has been build with the goal of easing the training of deep networks. The network design is based on the concept of chaining multiple residual learning block on a row. One residual leanring block can be seen in Figure \ref{fig:x resblock}. \cite{he_deep_2015}


\subsection{Image datasets}
\subsubsection{MNIST}
The MNIST database contains 70,000 28x28 black and white images. 60,000 images are for training and 10,000 images for testing. The images' portrait hand written numbers from 0 to 9. \cite{yann_lecun_mnist_nodate} Examples of the classes can be seen in Figure \ref{fig:x MNIST image samples}.
\begin{figure}[!htbp]
    \centering
...
The dataset used in this paper, both have a resolution of 28x28 pixels. On top, the Cifar10 dataset has 3 color channels, while the MNIST dataset only has one gray-channel.
This represents an issue, as the Neural Networks have initially been designed to work with the Microsoft ImageNet dataset. This dataset has 3 color channels and a resolution of 224x224 pixels.
