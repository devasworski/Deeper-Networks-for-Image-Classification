...
The project has five entry points, which are each a Jupiter Notebook file. Each notebook is dedicated to one Neural Network.
...
While the Keras version of the model was not executable on Google Colab, the PyTorch version did train.
...
One being the bad performance of the Adam optimizer, the other being the bad performance of all clasifiers on the bird, cat \& deer or dog classes if the CIFAR dataset.

While literature suggests that the Adam optimizer has worse generalization performance than SGD \cite{gupta_adam_nodate}, the results found in this experiment come at a surprise.
As the performance of the models with the SGD optimizer are very well, it could be argued, that this scenario is a special case, in which the Adam optimizer performs particularly bad.
The one exception to this is the ResNet-50 model, which performs similarly well using both the Adam and the SGD optimizer.

The second observation was the issues in distinguishing the bird, cat \& deer or dog classes if the CIFAR dataset. While the model all perform excellent on the other classes these classes seem to be more difficult to clasify.
While this could be an indication of a Minority class, this can be out rules as the dataset are pre-revisioned and each class is equality represented.
...
These leads to the conclusion, that these classes are too difficult to distinguishing for the models. 
The argument is, that this images often cointain to little information to distinguish them. 
To test this arguemnt a  small test group of 10 people has been asked individually after been show one picture each of the bird, cat \& deer category, given the 10 labels to classify them.
...
Based on these non representative results, it can be argued, that the results in the CIFAR dataset are very well.

Looking back at the initially assumption based of Figure \ref{fig:x mode size}, it can be said, that this did almost correspond to the finding.
The ResNet model performed overall the best and the GoogLeNet model performed the worst.
With the expection that the VGG-16 model performed 0.06\% better on the MNIST dataset compared to the ResNet model. This is within the margin of error.
\section{Conclusion}\label{C6}
While the results obtained in the experiment differed slightly from the initial assumption, the overall experiment can be considered successfully.
...
While the implementation in both Keras and PyTorch was successfully, the Keras implementation did cause issues when executing on the Google Colab GPU, which did cost a lot of time.
