...
Unfortunately, this model was too complex for the Google Colab environment, even using Colab Pro. Therefore, the model was modified using suggestions made in the Stack Overflow forum. \cite{gervais_answer_2019}
...
The rest of the model was not modified. This led to horrible results. Due to a lack of other options for training the model, except for Google Colab, this model results will not be further discussed and the original VGG-16 architecture will be part of the project folder.
...
\subsection{VGG-16}
As mentioned previously, this model is too big to be execution using Google Colab Pro and the only results that could be obtained, are from a heavily simplified version of the model, which expectedly performs very bed.
As this poor performance become evident very quickly, the model has only been trained on the CIFAR dataset using the SGD optimizer. The results can be seen in Tbale \ref{tab: VGG model accuracy}
...
\begin{figure}[!htbp]
    \centering
    \includegraphics[scale=0.08]{img/matrix_sample.png}
